...
function connectCloud(token, profile, userContext, endpoint = process.env.REALTIME_AGENT_LAB_CLOUD_ENDPOINT) {
    ...
    return new Promise((resolve, reject) => {
        if (!endpoint) {
            reject(new Error('Cloud endpoint is not configured. Set REALTIME_AGENT_LAB_CLOUD_ENDPOINT or use a BYOK provider.'));
            return;
        }
        const separator = endpoint.includes('?') ? '&' : '?';
        const url = `${endpoint}${separator}token=${encodeURIComponent(token)}`;
        console.log('[Cloud] Connecting to configured endpoint');
