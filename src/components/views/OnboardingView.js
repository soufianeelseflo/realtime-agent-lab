...
export class OnboardingView extends LitElement {
    ...
    async completeOnboarding() {
        if (this.contextText.trim()) {
            await realtimeAgentLab.storage.updatePreference('customPrompt', this.contextText.trim());
        }
        await realtimeAgentLab.storage.updateConfig('onboarded', true);
        this.onComplete();
    }
    ...
    renderSlide() {
        if (this.currentSlide === 0) {
            return html`
                <div class="slide">
                    <div class="slide-title">Realtime Agent Lab</div>
