...
export class MainView extends LitElement {
    ...
    async _loadFromStorage() {
        try {
            const [config, prefs, creds] = await Promise.all([
                realtimeAgentLab.storage.getConfig(),
                realtimeAgentLab.storage.getPreferences(),
                realtimeAgentLab.storage.getCredentials().catch(() => ({})),
            ]);
            ...
            if (storedMode === 'cloud') {
                await realtimeAgentLab.storage.updatePreference('providerMode', this._mode);
            }

            // Load keys
            this._token = creds.cloudToken || '';
            this._geminiKey = (await realtimeAgentLab.storage.getApiKey().catch(() => '')) || '';
            this._groqKey = (await realtimeAgentLab.storage.getGroqApiKey().catch(() => '')) || '';
    ...
    async _saveMode(mode) {
        ...
        this._tokenError = false;
        this._keyError = false;
        await realtimeAgentLab.storage.updatePreference('providerMode', mode);
        this.requestUpdate();
    }
    ...
    async _saveToken(val) {
        ...
        this._tokenError = false;
        try {
            const creds = await realtimeAgentLab.storage.getCredentials().catch(() => ({}));
            await realtimeAgentLab.storage.setCredentials({ ...creds, cloudToken: val });
        } catch (e) {}
        this.requestUpdate();
    }
    ...
    async _saveGeminiKey(val) {
        this._geminiKey = val;
        this._keyError = false;
        await realtimeAgentLab.storage.setApiKey(val);
        this.requestUpdate();
    }

    async _saveGroqKey(val) {
        this._groqKey = val;
        await realtimeAgentLab.storage.setGroqApiKey(val);
        this.requestUpdate();
    }

    async _saveGeminiLiveModel(val) {
        this._geminiLiveModel = val;
        await realtimeAgentLab.storage.updateConfig('geminiLiveModel', val);
        this.requestUpdate();
    }

    async _saveGroqModel(val) {
        this._groqModel = val;
        await realtimeAgentLab.storage.updateConfig('groqModel', val);
        this.requestUpdate();
    }

    async _saveGroqImageModel(val) {
        this._groqImageModel = val;
        await realtimeAgentLab.storage.updateConfig('groqImageModel', val);
        this.requestUpdate();
    }

    async _saveDisableGroqThinking(disabled) {
        this._disableGroqThinking = disabled;
        await realtimeAgentLab.storage.updateConfig('disableGroqThinking', disabled);
        this.requestUpdate();
    }
    ...
    async _saveOpenaiKey(val) {
        ...
        try {
            const creds = await realtimeAgentLab.storage.getCredentials().catch(() => ({}));
            await realtimeAgentLab.storage.setCredentials({ ...creds, openaiKey: val });
        } catch (e) {}
        this.requestUpdate();
    }
    ...
    async _saveLocalLlmModel(val) {
        ...
        await realtimeAgentLab.storage.updatePreference('localLlmModel', val);
        this.requestUpdate();
    }
    ...
    async _saveWhisperModel(val) {
        ...
        await realtimeAgentLab.storage.updatePreference('whisperModel', val);
        this.requestUpdate();
    }
            ...
            <div class="form-wrapper">
                ${
                    this._mode === 'local'
                        ? html`
                              <div class="title-row">
                                  <div class="page-title">Realtime Agent Lab <span class="mode-suffix">Local AI</span></div>
                                  <button class="help-btn" @click=${this._openLocalHelp} aria-label="Open Local AI help">${helpIcon}</button>
                              </div>
                          `
                        : html` <div class="page-title">${html`Realtime Agent Lab <span class="mode-suffix">BYOK</span>`}</div> `
                }
                    ...
                    <div class="help-content">
                        <div class="help-section">
                            ...
                            <div class="help-section-text">
                                Realtime Agent Lab runs llama.cpp and whisper.cpp directly. Everything stays on your computer — no external AI service or
                        ...
                        <div class="help-section">
                            ...
                            <div class="help-section-text">
                                ...
                                use. They are stored in the Realtime Agent Lab config directory.
                            </div>
                        </div>
