...
async function initializeGemini(profile = 'interview', language = 'en-US') {
    ...
    if (apiKey) {
        ...
        if (success) {
            realtimeAgentLab.setStatus('Live');
        } else {
            realtimeAgentLab.setStatus('error');
        }
    }
}
...
async function initializeLocal(profile = 'interview') {
    ...
    if (success) {
        realtimeAgentLab.setStatus('Local AI Live');
        return true;
    } else {
        realtimeAgentLab.setStatus('error');
        return false;
...
ipcRenderer.on('update-status', (event, status) => {
    ...
    realtimeAgentLab.setStatus(status);
});
        ...
        async blob => {
            ...
            reader.onloadend = async () => {
                ...
                if (result.success) {
                    ...
                    realtimeAgentLab.addNewResponse(`Error: ${result.error}`);
                }
            };
            reader.readAsDataURL(blob);
        },
...
function handleShortcut(shortcutKey) {
    const currentView = realtimeAgentLab.getCurrentView();

    if (shortcutKey === 'ctrl+enter' || shortcutKey === 'cmd+enter') {
        if (currentView === 'main') {
            realtimeAgentLab.element().handleStart();
        } else {
...
const realtimeAgentLabApp = document.querySelector('realtime-agent-lab-app');
...
// Consolidated realtimeAgentLab object - all functions in one place
const realtimeAgentLab = {
    // App version
    getVersion: async () => ipcRenderer.invoke('get-app-version'),

    // Element access
    element: () => realtimeAgentLabApp,
    e: () => realtimeAgentLabApp,

    // App state functions - access properties directly from the app element
    getCurrentView: () => realtimeAgentLabApp.currentView,
    getLayoutMode: () => realtimeAgentLabApp.layoutMode,

    // Status and response functions
    setStatus: text => realtimeAgentLabApp.setStatus(text),
    addNewResponse: response => realtimeAgentLabApp.addNewResponse(response),
    updateCurrentResponse: response => realtimeAgentLabApp.updateCurrentResponse(response),
...
window.realtimeAgentLab = realtimeAgentLab;
