...
export class RealtimeAgentLabApp extends LitElement {
    static styles = css`
        * {
    ...
    async _checkForUpdates() {
        try {
            this._localVersion = await realtimeAgentLab.getVersion();
            this.requestUpdate();

            const res = await fetch('https://raw.githubusercontent.com/sohzm/realtime-agent-lab/refs/heads/master/package.json');
            if (!res.ok) return;
    ...
    async _loadFromStorage() {
        try {
            const [config, prefs] = await Promise.all([realtimeAgentLab.storage.getConfig(), realtimeAgentLab.storage.getPreferences()]);
    ...
    async handleStart() {
        const prefs = await realtimeAgentLab.storage.getPreferences();
        const providerMode = prefs.providerMode === 'cloud' ? 'byok' : prefs.providerMode || 'byok';

        if (providerMode === 'cloud') {
            const creds = await realtimeAgentLab.storage.getCredentials();
            ...
            const success = await realtimeAgentLab.initializeCloud(this.selectedProfile);
            if (!success) {
                const mainView = this.shadowRoot.querySelector('main-view');
                if (mainView && mainView.triggerApiKeyError) {
                    mainView.triggerApiKeyError();
            ...
            const success = await realtimeAgentLab.initializeLocal(this.selectedProfile);
            if (!success) {
                const mainView = this.shadowRoot.querySelector('main-view');
                if (mainView && mainView.triggerApiKeyError) {
                    mainView.triggerApiKeyError();
            ...
            const apiKey = await realtimeAgentLab.storage.getApiKey();
            if (!apiKey || apiKey === '') {
                const mainView = this.shadowRoot.querySelector('main-view');
                if (mainView && mainView.triggerApiKeyError) {
                    mainView.triggerApiKeyError();
            ...
            await realtimeAgentLab.initializeGemini(this.selectedProfile, this.selectedLanguage);
        }

        realtimeAgentLab.startCapture(this.selectedScreenshotInterval, this.selectedImageQuality);
        this.responses = [];
        this.currentResponseIndex = -1;
    ...
    async handleCancelLocalDownload() {
        await realtimeAgentLab.cancelLocalInitialization();
    }

    async handleAPIKeyHelp() {
        if (window.require) {
            const { ipcRenderer } = window.require('electron');
            await ipcRenderer.invoke('open-external', 'https://github.com/YOUR_GITHUB_USERNAME/realtime-agent-lab#provider-support');
        }
    }
    ...
    async handleProfileChange(profile) {
        ...
        await realtimeAgentLab.storage.updatePreference('selectedProfile', profile);
    }

    async handleLanguageChange(language) {
        this.selectedLanguage = language;
        await realtimeAgentLab.storage.updatePreference('selectedLanguage', language);
    }

    async handleScreenshotIntervalChange(interval) {
        this.selectedScreenshotInterval = interval;
        await realtimeAgentLab.storage.updatePreference('selectedScreenshotInterval', interval);
    }

    async handleImageQualityChange(quality) {
        this.selectedImageQuality = quality;
        await realtimeAgentLab.storage.updatePreference('selectedImageQuality', quality);
    }

    async handleLayoutModeChange(layoutMode) {
        this.layoutMode = layoutMode;
        await realtimeAgentLab.storage.updateConfig('layout', layoutMode);
        this.requestUpdate();
    }
    ...
    async handleSendText(message) {
        const result = await window.realtimeAgentLab.sendTextMessage(message);
        if (!result.success) {
                ...
                <div class="sidebar-footer">
                    ${
                        this._updateAvailable
                            ? html`
                                  <button class="update-btn" @click=${() => this.handleExternalLinkClick('https://github.com/YOUR_GITHUB_USERNAME/realtime-agent-lab/releases')}>
...
customElements.define('realtime-agent-lab-app', RealtimeAgentLabApp);
