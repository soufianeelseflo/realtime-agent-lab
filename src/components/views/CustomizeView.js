...
export class CustomizeView extends LitElement {
    ...
    getThemes() {
        return realtimeAgentLab.theme.getAll();
    }

    async _loadFromStorage() {
        try {
            const [prefs, keybinds] = await Promise.all([realtimeAgentLab.storage.getPreferences(), realtimeAgentLab.storage.getKeybinds()]);
    ...
    getDefaultKeybinds() {
        const isMac = realtimeAgentLab.isMacOS || navigator.platform.includes('Mac');
        return {
    ...
    async saveKeybinds() {
        await realtimeAgentLab.storage.setKeybinds(this.keybinds);
        if (window.require) {
    ...
    async handleCustomPromptInput(e) {
        ...
        await realtimeAgentLab.storage.updatePreference('customPrompt', this.customPrompt);
    }

    async handleAudioModeSelect(e) {
        this.audioMode = e.target.value;
        await realtimeAgentLab.storage.updatePreference('audioMode', this.audioMode);
        this.requestUpdate();
    }

    async handleThemeChange(e) {
        this.theme = e.target.value;
        await realtimeAgentLab.theme.save(this.theme);
        this.updateBackgroundAppearance();
        this.requestUpdate();
    }
    ...
    async handleGoogleSearchChange(e) {
        ...
        await realtimeAgentLab.storage.updatePreference('googleSearchEnabled', this.googleSearchEnabled);
        if (window.require) {
            try {
                const { ipcRenderer } = window.require('electron');
    ...
    async handleBackgroundTransparencyChange(e) {
        ...
        await realtimeAgentLab.storage.updatePreference('backgroundTransparency', this.backgroundTransparency);
        this.updateBackgroundAppearance();
        this.requestUpdate();
    }

    updateBackgroundAppearance() {
        const colors = realtimeAgentLab.theme.get(this.theme);
        realtimeAgentLab.theme.applyBackgrounds(colors.background, this.backgroundTransparency);
    }

    async handleFontSizeChange(e) {
        this.fontSize = parseInt(e.target.value, 10);
        await realtimeAgentLab.storage.updatePreference('fontSize', this.fontSize);
        this.updateFontSize();
        this.requestUpdate();
    }
    ...
    async resetKeybinds() {
        this.keybinds = this.getDefaultKeybinds();
        await realtimeAgentLab.storage.setKeybinds(null);
        if (window.require) {
            const { ipcRenderer } = window.require('electron');
            ipcRenderer.send('update-keybinds', this.keybinds);
        }
    ...
    async restoreAllSettings() {
        ...
        try {
            ...
            for (const [key, value] of Object.entries(defaults)) {
                await realtimeAgentLab.storage.updatePreference(key, value);
            }
            ...
            this.updateBackgroundAppearance();
            this.updateFontSize();
            await realtimeAgentLab.theme.save(defaults.theme);
    ...
    async clearLocalData() {
        ...
        this.requestUpdate();
        try {
            await realtimeAgentLab.storage.clearAll();
